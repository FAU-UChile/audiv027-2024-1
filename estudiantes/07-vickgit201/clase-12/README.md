...
apuntes:
CNN reconocimiento de caras y detección de objetos (mejor)(computer vision application)
GAN después la CNN
Puede generar sin instrucciones 

Características de una GAN: Generación de imágenes realistas:Mejora de imágenes y super-resolución;Transferencia de estilo y síntesis de texturas

Generación de imágenes realistas
Las GAN 's han demostrado ser excepcionalmente eficaces en la generación de imágenes realistas. Utilizando un generador y un discriminador en competencia, las GAN 's pueden crear imágenes que son indistinguibles de las imágenes reales en muchos casos. Esto tiene aplicaciones significativas en áreas como la creación de contenido digital, diseño asistido por computadora, y simulaciones realistas para entrenamiento en realidad virtual.

Mejora de imágenes y super-resolución
Otro uso poderoso de las GANs es mejorar la calidad de las imágenes existentes y realizar super-resolución. Mediante el entrenamiento con conjuntos de datos de alta y baja resolución, las GAN ‘s pueden generar versiones mejoradas de imágenes con detalles más nítidos y claros. Esta capacidad es valiosa en aplicaciones como la mejora de imágenes médicas, restauración de imágenes históricas, y en sistemas de vigilancia y análisis de imágenes.

Transferencia de estilo y síntesis de texturas
Las GANs también son utilizadas para la transferencia de estilo, donde el estilo de una imagen de origen se aplica a otra imagen de destino, preservando la estructura y los contenidos generales de la imagen original. Además, las GANs pueden sintetizar texturas que son perceptualmente realistas y coherentes, lo cual es esencial en áreas como el diseño gráfico, la animación y la creación de contenido multimedia.


Arquitectura básica de una GAN

apuntes:
Generador y discriminador creados como adversarios
Arquitectura del generador:
The Generator typically consists of one or more deep neural networks, often using convolutional layers to generate images or recurrent layers to generate sequential data.
The generator architecture comprises three components: the latent space, the generator, and the image 4/16 generation section
The generator samples from the latent space and creates a relationship between the latent space and the output. We then create a neural network that maps from the input (latent space) to the output (image for most examples)
Espacio latente, ruido random 
